...
module.exports  = {
     creates:(data, callBack) => {
        pool.query(
            ...
            (err,results) =>{
                ...
                if(results == ""){
                    pool.query(
                        `insert into volunteer(name,contact,email,gender,date_of_birth,address,profession,reason,date)values(?,?,?,?,?,?,?,?,?)`,
                         [
                            data.name,
                            data.contact,
                            data.email,
     ...
     getsById : (id, callBack) => {
        pool.query(`select * from volunteer where id = ?`,
        [id],
        (err,results) => {
            if(err){
                return callBack(err);
     ...
     deletesById:(id,callBack) => {
        pool.query(`delete from volunteer where id=?`,
            [ 
                id
            ],        
            (err,results) => {
